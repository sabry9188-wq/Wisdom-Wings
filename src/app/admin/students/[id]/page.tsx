...
import { AttendanceBadge, Badge, FeeStatusBadge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { formatDate, formatCurrency } from "@/lib/utils";
import { ToggleActiveButton } from "@/components/students/toggle-active-button";
...
export default async function StudentDetailsPage({
  ...
  const [{ data: guardians }, { data: attendance }, { data: fees }] =
    await Promise.all([
      supabase
        .from("parent_student")
        .select("relationship, is_primary_contact, users(id, full_name, phone, email)")
        .eq("student_id", id),
      supabase
        .from("attendance")
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            ...
            <ToggleActiveButton studentId={id} isActive={student.is_active} />
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          <CardBody className="flex flex-col items-center text-center">
            ...
            <Badge tone={student.is_active ? "green" : "slate"}>
            ...
            <dl className="mt-4 w-full space-y-2 text-left text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd className="text-slate-900">{student.contact_phone ?? "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-slate-500">Address</dt>
                <dd className="text-slate-900">{student.contact_address ?? "—"}</dd>
              </div>
            </dl>
          </CardBody>
        </Card>
          ...
          <Card>
            ...
            <CardBody className="p-0">
              {guardians && guardians.length > 0 ? (
                ...
                <EmptyState message="No guardians linked yet. Link one from the Parents page." />
              )}
            </CardBody>
          </Card>
